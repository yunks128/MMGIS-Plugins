...
import LayersTool from '../../../core/tools/Layers/LayersTool'
import LegendTool from '../../../core/tools/Legend/LegendTool'
...
function buildVesselLiveQuery(payload) {
    const params = new URLSearchParams()
    if (payload?.types) {
        const t = Array.isArray(payload.types)
            ? payload.types.join(',')
            : String(payload.types)
        if (t.trim()) params.set('types', t)
    }
    if (payload?.flags) {
        const f = Array.isArray(payload.flags)
            ? payload.flags.join(',')
            : String(payload.flags)
        if (f.trim()) params.set('flags', f)
    }
    if (Array.isArray(payload?.bounds) && payload.bounds.length === 4) {
        params.set('bounds', payload.bounds.join(','))
    }
    return params.toString()
}

async function applyVesselLayerFilters(payload) {
    const id = resolveDisplayNameToId(VESSEL_LAYER_NAME)
    if (!id) throw new Error(`Layer "${VESSEL_LAYER_NAME}" not found.`)
    const data = L_.layers.data[id]
    if (!data) throw new Error('Vessel layer data unavailable.')
    const qs = buildVesselLiveQuery(payload)
    data.url = qs ? `/api/vessels/live?${qs}` : '/api/vessels/live'
    if (typeof L_.refreshLayer === 'function') {
        await L_.refreshLayer(data)
    } else if (window.mmgisAPI?.refreshLayer) {
        await window.mmgisAPI.refreshLayer(id)
    }
}

export async function render_show_vessels(_ctx, payload) {
    const visible = payload?.visible !== false
    const id = resolveDisplayNameToId(VESSEL_LAYER_NAME)
    if (!id) throw new Error(`Layer "${VESSEL_LAYER_NAME}" not found.`)
    if (visible && (payload?.types || payload?.flags || payload?.bounds)) {
        await applyVesselLayerFilters(payload)
    }
    await window.mmgisAPI.toggleLayer(id, visible)
    appendLine(
        visible
            ? `Vessels layer enabled${
                  payload?.types ? ` (types: ${payload.types})` : ''
              }${payload?.flags ? ` (flags: ${payload.flags})` : ''}.`
            : 'Vessels layer hidden.'
    )
}

export async function render_filter_vessels_by_type(_ctx, payload) {
    if (!payload?.types) {
        throw new Error('filter_vessels_by_type requires a "types" parameter.')
    }
    await applyVesselLayerFilters(payload)
    const id = resolveDisplayNameToId(VESSEL_LAYER_NAME)
    if (id) await window.mmgisAPI.toggleLayer(id, true)
    const list = Array.isArray(payload.types)
        ? payload.types.join(', ')
        : payload.types
    appendLine(`Filtered vessels to: ${list}.`)
}

export async function render_vessel_count_in_region(_ctx, payload) {
    const qs = buildVesselLiveQuery(payload)
    const url = qs ? `/api/vessels/live?${qs}` : '/api/vessels/live'
    const res = await fetch(url)
    if (!res.ok) throw new Error(`Vessel count fetch failed: HTTP ${res.status}`)
    const fc = await res.json()
    const count = (fc.features || []).length
    const filterDesc = []
    if (payload?.types) filterDesc.push(`types=${payload.types}`)
    if (payload?.flags) filterDesc.push(`flags=${payload.flags}`)
    if (payload?.bounds) filterDesc.push(`bounds=${payload.bounds.join(',')}`)
    appendLine(
        `${count} vessels${
            filterDesc.length ? ` matching ${filterDesc.join(', ')}` : ''
        }.`
    )
}

export async function render_show_vessels_in_high_ice(_ctx, payload) {
    ...
    const res = await fetch(`/api/vessels/in-ice?${params.toString()}`)
    if (!res.ok) {
        throw new Error(`in-ice fetch failed: HTTP ${res.status}`)
    }
    const fc = await res.json()
    const meta = fc._meta || {}
    const features = fc.features || []
    const group = ensureOverlayGroup(VESSEL_HIGH_ICE_OVERLAY_KEY)
    features.forEach((f) => {
        const c = f.geometry?.coordinates
        if (!Array.isArray(c) || c.length < 2) return
        const p = f.properties || {}
        window.L.circleMarker([c[1], c[0]], {
            radius: 7,
            color: '#dc2626',
            weight: 2,
            fillColor: '#fecaca',
            ...
            .bindPopup(
                `<b>${p.name || p.mmsi}</b><br/>` +
                    `Flag: ${p.flagCountry || p.flag || '-'}<br/>` +
                    `Ice: ${(p.iceFraction != null
                        ? Math.round(p.iceFraction * 100)
                        : '?')}%<br/>` +
                    `Speed: ${p.speed ?? '-'} kn`
            )
            .addTo(group)
    ...
    appendLine(
        `${features.length} vessels in ≥${threshold}% sea-ice on ${
            meta.forecastDate || '?'
        } (of ${meta.vesselsTotal ?? '?'} total).`
    )
}

export async function render_show_vessel_track(_ctx, payload) {
    let mmsi = payload?.mmsi
    if (!mmsi && payload?.name) {
        // Resolve vessel by name from cached layer features
        const id = resolveDisplayNameToId(VESSEL_LAYER_NAME)
        const data = id ? L_.layers.data[id] : null
        const features = data?._lastGeoJson?.features || []
        const target = String(payload.name).trim().toLowerCase()
        const match = features.find(
            (f) => (f.properties?.name || '').toLowerCase() === target
        )
        if (match) mmsi = match.properties.mmsi
    }
    if (!mmsi) {
        throw new Error('show_vessel_track requires "mmsi" or matching "name".')
    }
    ...
    if (coords.length < 2) {
        appendLine(`No track points yet for MMSI ${mmsi}.`)
        return
    }
    const group = ensureOverlayGroup(VESSEL_TRACK_OVERLAY_KEY)
    ...
    appendLine(
        `Track for MMSI ${mmsi}: ${latlngs.length} points over ~${hours}h.`
    )
}
...
function rebindVesselPopups() {
    try {
        ...
        // Layer key in L_.layers.layer may be either the display name or the
        // resolved UUID — try the resolved ID first, then fall back.
        const id =
            (typeof resolveDisplayNameToId === 'function' &&
                resolveDisplayNameToId(VESSEL_LAYER_NAME)) ||
            VESSEL_LAYER_NAME
        let leafletLayer =
            L_.layers.layer[id] || L_.layers.layer[VESSEL_LAYER_NAME]
        // Last resort: scan L_.layers.layer for any FeatureGroup whose features
        // carry an mmsi property.
        if (!leafletLayer || typeof leafletLayer.eachLayer !== 'function') {
            for (const k of Object.keys(L_.layers.layer)) {
        ...
        if (count > 0 && !window.__vesselPopupsLoggedOnce) {
            console.log(`[Vessels] bound rich popups on ${count} markers (subsequent rebinds silent)`)
    ...
    const tryInstall = () => {
        ...
        map.on('popupopen', (e) => {
            try {
                ...
                if (
                    layer?.options?.layerName === VESSEL_LAYER_NAME &&
                    feature?.properties?.mmsi
                ) {
                    render_show_vessel_track(null, {
        ...
        rebindVesselPopups()
        try {
            L_.subscribeOnLayerToggle &&
                L_.subscribeOnLayerToggle('vesselPopups', (name, off) => {
                    if (name === VESSEL_LAYER_NAME && !off) {
                        setTimeout(rebindVesselPopups, 200)
                    }
                })
            L_.subscribeTimeLayerReloadFinish &&
                L_.subscribeTimeLayerReloadFinish('vesselPopups', () => {
                    setTimeout(rebindVesselPopups, 100)
                })
        } catch (_) {}
        // The reload-finish subscription only fires for `geodatasets:` URLs in
        // MMGIS core, so for our /api/vessels/live URL the 30s auto-refresh
        // and time-slider requery silently re-create markers without popups.
        // A cheap periodic rebind makes us robust against any reload path.
        // rebindVesselPopups() is idempotent (skips markers that already have
        // a popup) so the cost is just an eachLayer walk.
        setInterval(rebindVesselPopups, 2000)
        // Also poll for ~10s to catch late layer creation (e.g. layer toggled
        // on after this handler installed but before reload events flow).
        let tries = 0
        const t = setInterval(() => {
            tries += 1
...
// MMGIS bootstrap (which was hanging when the installer ran synchronously).
if (typeof window !== 'undefined') {
...
export const RENDERERS = {
    ...
    analyzable_layers: list_analyzable_layers,
    text_with_citation: render_text_with_citation,
    links_summary: render_links_summary,
    zoom_view,
    set_visible_layers_time,
    set_opacity,
    toggle_visibility,
    layer_information: render_layer_information,
    layer_mean: render_layer_mean,
    local_layer_mean: render_local_calculate_mean,
    local_layer_minmax: render_local_calculate_minmax,
    local_layer_histogram: render_local_calculate_histogram,
    local_layer_std: render_local_calculate_std,
    local_threshold_mask: render_local_threshold_mask,
...
RENDERERS.render_list_analyzable_layers = list_analyzable_layers
...
RENDERERS.render_layer_summary = render_layer_summary
RENDERERS.render_anomaly_detection = render_anomaly_detection
RENDERERS.render_multilayer_statistics = render_multilayer_statistics
RENDERERS.render_temporal_trends = render_temporal_trends
RENDERERS.render_spatial_statistics = render_spatial_statistics
RENDERERS.render_change_detection = render_change_detection
RENDERERS.render_time_series_animation = render_time_series_animation
RENDERERS.render_cross_section = render_cross_section
RENDERERS.render_data_export = render_data_export
RENDERERS.render_set_visible_layers_time = set_visible_layers_time
RENDERERS.render_opacity = set_opacity
RENDERERS.render_toggle = toggle_visibility

export async function render_set_layer_color_scale(_ctx, payload) {
    const layerName =
        payload?.layer_name || payload?.name || payload?.layer
    const minVal = Number(payload?.min)
    const maxVal = Number(payload?.max)
    if (!layerName) {
        appendLine('set_layer_color_scale requires a layer name.')
        return
    }
    if (!Number.isFinite(minVal) || !Number.isFinite(maxVal)) {
        appendLine('set_layer_color_scale requires numeric min and max.')
        return
    }
    const index = buildLayerIndex()
    const match = findLayerMatch(layerName, index)
    if (!match) {
        appendLine(`Could not match layer: ${layerName}`)
        return
    }
    const live = match.layer?.liveInstance
    if (!live || !live.options) {
        appendLine(
            `Layer "${match.displayName}" has no live tile instance. Toggle it on first.`
        )
        return
    }
    live.options.currentCogMin = minVal
    live.options.currentCogMax = maxVal
    if (match.layer) {
        match.layer.currentCogMin = minVal
        match.layer.currentCogMax = maxVal
    }
    // Use the same refresh path the layer-settings input uses, so tiles re-fetch
    // with the new rescale.
    try {
        if (
            L_?.layers?.layer?.[match.layer.name] &&
            typeof L_.layers.layer[match.layer.name].refresh === 'function'
        ) {
            L_.layers.layer[match.layer.name].refresh(null, true, {
                currentCogMin: minVal,
                currentCogMax: maxVal,
            })
        } else if (typeof live.redraw === 'function') {
            live.redraw()
        }
    } catch (_) {
        if (typeof live.redraw === 'function') live.redraw()
    }
    // Refresh the layer panel colorbar labels and the LegendTool legend.
    // populateCogScale bails when layer.url lacks a "stac-collection:" prefix,
    // even for stac-collection sourceType layers, so prefix it transiently.
    try {
        const dataEntry =
            L_?.layers?.data?.[L_.asLayerUUID(match.layer.name)] ||
            match.layer
        // Push min/max onto the canonical L_.layers.data entry as well.
        dataEntry.currentCogMin = minVal
        dataEntry.currentCogMax = maxVal
        // Clear cached _legend so refreshLegends re-runs populateCogScale.
        delete dataEntry._legend
        if (LayersTool && typeof LayersTool.populateCogScale === 'function') {
            const origUrl = dataEntry.url
            const needsPrefix =
                dataEntry.sourceType === 'stac-collection' &&
                typeof origUrl === 'string' &&
                !origUrl.startsWith('stac-collection:') &&
                !origUrl.startsWith('COG:')
            if (needsPrefix) dataEntry.url = `stac-collection:${origUrl}`
            try {
                LayersTool.populateCogScale(dataEntry.name)
            } finally {
                if (needsPrefix) dataEntry.url = origUrl
            }
        }
    } catch (e) {
        try { console.warn('populateCogScale failed:', e) } catch (_) {}
    }
    try {
        if (LegendTool && typeof LegendTool.refreshLegends === 'function') {
            LegendTool.refreshLegends()
        }
    } catch (_) {}
    // Sync the inline input fields, if the layer settings panel is open.
    try {
        $(`.tilerescalecogmin[layername="${match.layer.name}"]`).val(minVal)
        $(`.tilerescalecogmax[layername="${match.layer.name}"]`).val(maxVal)
    } catch (_) {}
    appendLine(
        `Set color scale of **${match.displayName}** to [${minVal}, ${maxVal}].`
    )
}
RENDERERS.set_layer_color_scale = render_set_layer_color_scale
RENDERERS.render_set_layer_color_scale = render_set_layer_color_scale

export async function render_set_current_time(_ctx, payload) {
    const api = typeof window !== 'undefined' ? window.mmgisAPI : null
    if (!api || typeof api.setTime !== 'function') {
        appendLine('Time control API not available.')
        return
    }
    const raw = payload?.time || payload?.date || payload?.iso
    if (!raw) {
        appendLine('set_current_time requires a time/date string.')
        return
    }
    ...
    if (Number.isNaN(parsed.getTime())) {
        appendLine(`Could not parse time string: ${raw}`)
        return
    }
    const targetIso = parsed.toISOString()

    // Preserve the current window unless the target falls outside it.
    let startIso = (typeof api.getStartTime === 'function' && api.getStartTime()) || targetIso
    let endIso = (typeof api.getEndTime === 'function' && api.getEndTime()) || targetIso
    const startMs = new Date(startIso).getTime()
    const endMs = new Date(endIso).getTime()
    const targetMs = parsed.getTime()
    let widened = false
    if (Number.isFinite(startMs) && targetMs < startMs) {
        startIso = targetIso
        widened = true
    }
    if (Number.isFinite(endMs) && targetMs > endMs) {
        endIso = targetIso
        widened = true
    }

    try {
        api.setTime(startIso, endIso, false, '00:00:00', targetIso)
        const note = widened ? ' (window extended to include target)' : ''
        appendLine(`Time set to **${targetIso}**${note}.`)
    } catch (e) {
        appendLine(`Failed to set time: ${e?.message || e}`)
    }
}
RENDERERS.set_current_time = render_set_current_time
RENDERERS.render_set_current_time = render_set_current_time

export async function render_set_animation_speed(_ctx, payload) {
    if (!TimeUI || !Array.isArray(TimeUI.intervalValues)) {
        appendLine('Time control not available.')
        return
    }
    const values = TimeUI.intervalValues
    const names = TimeUI.intervalNames || []
    const direction = String(payload?.direction || '').toLowerCase()
    const intervalMs = Number(payload?.interval_ms)
    const stepHint = Math.max(1, Math.abs(Number(payload?.step) || 1))
    let newIndex = Number.isInteger(TimeUI.intervalIndex)
        ? TimeUI.intervalIndex
        : 0

    if (direction === 'faster' || direction === 'speed_up') {
        newIndex = Math.max(0, newIndex - stepHint)
    } else if (direction === 'slower' || direction === 'slow_down') {
        newIndex = Math.min(values.length - 1, newIndex + stepHint)
    } else if (Number.isFinite(intervalMs) && intervalMs > 0) {
        let best = 0
        let bestDist = Math.abs(values[0] - intervalMs)
        for (let i = 1; i < values.length; i += 1) {
            const d = Math.abs(values[i] - intervalMs)
            if (d < bestDist) {
                bestDist = d
                best = i
            }
        ...
        appendLine(
            'set_animation_speed needs direction ("faster"/"slower") or interval_ms.'
        )
        return
    }

    TimeUI.intervalIndex = newIndex
    if (typeof TimeUI._refreshIntervals === 'function')
        TimeUI._refreshIntervals()
    if (typeof TimeUI._refreshLiveProgress === 'function')
        TimeUI._refreshLiveProgress()
    const ms = values[newIndex]
    const label = names[newIndex] || `${ms} ms`
    appendLine(`Animation rate set to **${label}** (${ms} ms per step).`)
}
RENDERERS.set_animation_speed = render_set_animation_speed
RENDERERS.render_set_animation_speed = render_set_animation_speed

export async function render_play_animation(_ctx, payload) {
    const force = typeof payload?.play === 'boolean' ? payload.play : true
    const btn = document.getElementById('mmgisTimeUIPlay')
    if (!btn) {
        appendLine('Time control play button not found.')
        return
    }
    const w = typeof window !== 'undefined' ? window : null

    // When *starting* playback, default the rate to 500 ms per frame unless
    // the caller explicitly asked for something else.
    if (force && TimeUI && Array.isArray(TimeUI.intervalValues)) {
        const requested = Number(payload?.interval_ms)
        const target = Number.isFinite(requested) && requested > 0 ? requested : 500
        let bestIdx = 0
        let bestDist = Math.abs(TimeUI.intervalValues[0] - target)
        for (let i = 1; i < TimeUI.intervalValues.length; i += 1) {
            const d = Math.abs(TimeUI.intervalValues[i] - target)
            if (d < bestDist) {
                bestDist = d
                bestIdx = i
            }
        }
        if (TimeUI.intervalIndex !== bestIdx) {
            TimeUI.intervalIndex = bestIdx
            try {
                if (typeof TimeUI._refreshIntervals === 'function')
                    TimeUI._refreshIntervals()
                if (typeof TimeUI._refreshLiveProgress === 'function')
    ...
    try {
        if (TimeUI && typeof TimeUI.togglePlay === 'function') {
            TimeUI.togglePlay(force)
        } else if (w?.$) {
            w.$('#mmgisTimeUIPlay').trigger('click')
        } else {
            btn.click()
        }
        const ms = TimeUI?.intervalValues?.[TimeUI?.intervalIndex]
        const rateLabel = Number.isFinite(ms) ? ` at ${ms} ms/frame` : ''
        appendLine(force ? `Started playback${rateLabel}.` : 'Paused playback.')
    } catch (e) {
        appendLine(`Failed to toggle playback: ${e?.message || e}`)
    }
}
RENDERERS.play_animation = render_play_animation
RENDERERS.render_play_animation = render_play_animation

// Vessels
RENDERERS.show_vessels = render_show_vessels
RENDERERS.render_show_vessels = render_show_vessels
RENDERERS.filter_vessels_by_type = render_filter_vessels_by_type
RENDERERS.render_filter_vessels_by_type = render_filter_vessels_by_type
RENDERERS.vessel_count_in_region = render_vessel_count_in_region
RENDERERS.render_vessel_count_in_region = render_vessel_count_in_region
RENDERERS.show_vessels_in_high_ice = render_show_vessels_in_high_ice
RENDERERS.render_show_vessels_in_high_ice = render_show_vessels_in_high_ice
RENDERERS.show_vessel_track = render_show_vessel_track
RENDERERS.render_show_vessel_track = render_show_vessel_track
