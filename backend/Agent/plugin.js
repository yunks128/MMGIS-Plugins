...
function getDynamicLayerInfo(missionPath) {
  ...
  try {
    ...
    store.error = { message: error.message, code: error.code };
    // Fallback to old static file if mission config fails
    return loadLayerInfoFromDisk(LAYER_INFO_PATH);
  }
...
let setup = {
  ...
  onceInit: (s) => {
    ...
    s.app.use((req, res, next) => {
      ...
      if (isStale) {
        const missionPath = path.join(process.cwd(), "Missions", mission);
